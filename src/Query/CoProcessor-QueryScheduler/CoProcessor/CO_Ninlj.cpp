  ...
  while (1) {
    ...
    *(pData->curPosToJoin) = toPos;
    ...
    if (execMode == EXEC_CPU) {
      ...
      if (realBlockSize > 0)
        numResult = CPU_ninlj(R + fromPos, realBlockSize, S, sLen, &tempRout,
                              OMP_JOIN_NUM_THREAD);
    ...
    } else // on the GPU
    {
      ...
      if (realBlockSize > 0)
        numResult =
            GPUCopy_ninlj(R + fromPos, realBlockSize, S, sLen, &tempRout);
...
int MergeJoinResult(vector<Record **> *tempResultVec, vector<int> *tempSizeVec,
                    Record **Rout) {
  int resultSize = 0;
...
int CO_ninlj(Record *R, int rLen, Record *S, int sLen, Record **Rout) {
  ...
  cout << "total: " << rLen << "->" << cpuBlockSize << ", " << gpuBlockSize
       << endl;
  for (i = 0; i < numThread; i++) {
    // Allocate memory for thread data.
    pData[i] = (ws_coninlj *)calloc(1, sizeof(ws_coninlj));
    ...
    if (i == 0) // GPU first.
    {
      pData[i]->init(R, rLen, gpuBlockSize, S, sLen, dispatchMutex, mergeMutex,
                     &curP, EXEC_GPU, tempResultVec, tempSizeVec);
      /*ON_GPU("tp_ninlj2");
      clock_t tt=0;
      ...
      pData[i]->init(R, rLen, cpuBlockSize, S, sLen, dispatchMutex, mergeMutex,
                     &curP, EXEC_CPU, tempResultVec, tempSizeVec);
    }
