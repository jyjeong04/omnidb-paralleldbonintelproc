...
GroupByThreadOp::GroupByThreadOp(OP_MODE opt) : SingularThreadOp(opt) {
  numGroup = -1;
  startPos = NULL;
}
...
void AggAfterGroupByThreadOp::init(cl_mem p_R, int p_rLen,
                                   cl_mem pRHavingGroupBy,
                                   int prLenHavingGroupBy, int numG,
                                   cl_mem pStartPos) {
  R = p_R;
  Query_rLen = p_rLen;
  numGroup = numG;
