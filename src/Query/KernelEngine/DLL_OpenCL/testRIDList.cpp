...
void setRIDListImpl(cl_mem d_RIDList, cl_mem d_tempOutput, int delta, int rLen,
                    cl_mem d_destRin, int numThreadPB, int numBlock, int *index,
                    cl_event *eventList, cl_kernel *Kernel, int *Flag_CPU_GPU,
                    double *burden, int _CPU_GPU)
// setRIDListImpl(d_RIDList, d_tempOutput, numThreadPerBlock,numBlock, rLen,
// d_destRin, CPU_GPU);
{
  ...
  cl_int ciErr1 =
      clSetKernelArg((*Kernel), 0, sizeof(cl_mem), (void *)&d_RIDList);
  ...
  kernel_enqueue(rLen, 10, 1, &globalWorkingSetSize, &numThreadsPerBlock_x,
                 eventList, index, Kernel, Flag_CPU_GPU, burden, _CPU_GPU);
}
void getRIDListImpl(cl_mem d_Rin, int delta, int rLen, cl_mem d_RIDList,
  ...
  kernel_enqueue(rLen, 11, 1, &globalWorkingSetSize, &numThreadsPerBlock_x,
                 eventList, index, Kernel, Flag_CPU_GPU, burden, _CPU_GPU);
}
