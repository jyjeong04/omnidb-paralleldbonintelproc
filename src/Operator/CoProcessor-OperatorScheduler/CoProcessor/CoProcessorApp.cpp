...
pthread_mutex_t OP_GPUBurdenCS = PTHREAD_MUTEX_INITIALIZER;
pthread_mutex_t OP_CPUBurdenCS = PTHREAD_MUTEX_INITIALIZER;
...
void usage(int argc, char **argv) {
  fprintf(stderr,
          "Usage: %s <total amount of querys><total number of threads>\n",
          argv[0]);
  fprintf(stderr, "\t<total amount of querys>		  	- side size of "
                  "matrix n (positive integer)\n");
  fprintf(stderr, "\t<total number of threads>			- number of "
                  "thread per block(1 - 32)\n");
  //	exit(1);
...
int main(int argc, char **argv) {
  ...
  pthread_mutex_destroy(&(OP_GPUBurdenCS));
  pthread_mutex_destroy(&(OP_CPUBurdenCS));
