...
void PROJECTION_HandShake(int CPU_GPU) {
  ...
  if (CPU_GPU) {
    ...
    printf("sum is %lf, CL_Projection invocatio overhead in average in GPU, "
           "%lf\n",
           sum, AddGPUBurden_OP[oid]);
  } else {
    AddCPUBurden_OP[oid] = sum / OCCount * scaler;
    printf("sum is %lf, CL_Projection invocatio overhead in average in CPU, "
           "%lf\n",
...
void SELECTION_HandShake(int CPU_GPU) {
  ...
  for (i = 0; i < OCCount; i++) {
    int timer = genTimer(oid);
    getTimer(timer);
    CL_RangeSelectionOnly(din, OP_rLen, smallKey, largeKey, &dout1, 256, 64,
                          CPU_GPU);
    double t = getTimer(timer);
    sum += t;
#ifdef HandshakeDebug
  ...
  if (CPU_GPU) {
    AddGPUBurden_OP[oid] = sum / OCCount * scaler;
    printf("sum is %lf, CL_RangeSelection invocatio overhead in average in "
           "GPU, %lf\n",
           sum, AddGPUBurden_OP[oid]);
  } else {
    AddCPUBurden_OP[oid] = sum / OCCount * scaler;
...
void AGG_SUM_HandShake(int CPU_GPU) {
  ...
  if (CPU_GPU) {
    AddGPUBurden_OP[oid] = sum / OCCount * scaler;
    printf(
        "sum is %lf, CL_AggSum invocatio overhead in average in GPU, %lf\n",
        sum, AddGPUBurden_OP[oid]);
  } else {
    AddCPUBurden_OP[oid] = sum / OCCount * scaler;
    printf(
        "sum is %lf, CL_AggSum invocatio overhead in average in CPU, %lf\n",
        sum, AddCPUBurden_OP[oid]);
  }
}
void AGG_MAX_HandShake(int CPU_GPU) {
  ...
  if (CPU_GPU) {
    AddGPUBurden_OP[oid] = sum / OCCount * scaler;
    printf(
        "sum is %lf, CL_AggMax invocatio overhead in average in GPU, %lf\n",
        sum, AddGPUBurden_OP[oid]);
  } else {
    AddCPUBurden_OP[oid] = sum / OCCount * scaler;
    printf(
        "sum is %lf, CL_AggMax invocatio overhead in average in CPU, %lf\n",
        sum, AddCPUBurden_OP[oid]);
  }
}
void AGG_MIN_HandShake(int CPU_GPU) {
  ...
  if (CPU_GPU) {
    AddGPUBurden_OP[oid] = sum / OCCount * scaler;
    printf(
        "sum is %lf, CL_AggMin invocatio overhead in average in GPU, %lf\n",
        sum, AddGPUBurden_OP[oid]);
  } else {
    AddCPUBurden_OP[oid] = sum / OCCount * scaler;
    printf(
        "sum is %lf, CL_AggMin invocatio overhead in average in CPU, %lf\n",
        sum, AddCPUBurden_OP[oid]);
  }
}
void AGG_AVG_HandShake(int CPU_GPU) {
  ...
  if (CPU_GPU) {
    AddGPUBurden_OP[oid] = sum / OCCount * scaler;
    printf(
        "sum is %lf, CL_AggAvg invocatio overhead in average in GPU, %lf\n",
        sum, AddGPUBurden_OP[oid]);
  } else {
    AddCPUBurden_OP[oid] = sum / OCCount * scaler;
    printf(
        "sum is %lf, CL_AggAvg invocatio overhead in average in CPU, %lf\n",
        sum, AddCPUBurden_OP[oid]);
  }
}
void AGG_SUM_AFTER_GROUP_BY_HandShake(int CPU_GPU) {
  ...
  for (i = 0; i < OCCount; i++) {
    ...
    getTimer(timer);
    CL_agg_sum_afterGroupBy(Rout, OP_rLen, startPos, numGroups, Rin2,
                            h_aggResults, 256, CPU_GPU);
    double t = getTimer(timer);
    sum += t;
#ifdef HandshakeDebug
  ...
  if (CPU_GPU) {
    AddGPUBurden_OP[oid] = sum / OCCount * scaler;
    printf("sum is %lf, CL_agg_sum_afterGroupBy invocatio overhead in "
           "average in GPU, %lf\n",
           sum, AddGPUBurden_OP[oid]);
  } else {
    AddCPUBurden_OP[oid] = sum / OCCount * scaler;
...
void AGG_MAX_AFTER_GROUP_BY_HandShake(int CPU_GPU) {
  ...
  for (i = 0; i < OCCount; i++) {
    ...
    int timer = genTimer(1);
    getTimer(timer);
    CL_agg_max_afterGroupBy(Rout, OP_rLen, startPos, numGroups, Rin2,
                            h_aggResults, 256, CPU_GPU);
    double t = getTimer(timer);
    sum += t;
#ifdef HandshakeDebug
  ...
  if (CPU_GPU) {
    AddGPUBurden_OP[oid] = sum / OCCount * scaler;
    printf("sum is %lf, CL_agg_max_afterGroupBy invocatio overhead in "
           "average in GPU, %lf\n",
           sum, AddGPUBurden_OP[oid]);
  } else {
    AddCPUBurden_OP[oid] = sum / OCCount * scaler;
...
void AGG_AVG_AFTER_GROUP_BY_HandShake(int CPU_GPU) {
  ...
  for (i = 0; i < OCCount; i++) {
    ...
    int timer = genTimer(1);
    getTimer(timer);
    CL_agg_avg_afterGroupBy(Rout, OP_rLen, startPos, numGroups, Rin2,
                            h_aggResults, 256, CPU_GPU);
    double t = getTimer(timer);
    sum += t;
#ifdef HandshakeDebug
  ...
  if (CPU_GPU) {
    AddGPUBurden_OP[oid] = sum / OCCount * scaler;
    printf("sum is %lf, CL_agg_avg_afterGroupBy invocatio overhead in "
           "average in GPU, %lf\n",
           sum, AddGPUBurden_OP[oid]);
  } else {
    AddCPUBurden_OP[oid] = sum / OCCount * scaler;
...
void AGG_MIN_AFTER_GROUP_BY_HandShake(int CPU_GPU) {
  ...
  for (i = 0; i < OCCount; i++) {
    ...
    int timer = genTimer(1);
    getTimer(timer);
    CL_agg_min_afterGroupBy(Rout, OP_rLen, startPos, numGroups, Rin2,
                            h_aggResults, 256, CPU_GPU);
    double t = getTimer(timer);
    sum += t;
#ifdef HandshakeDebug
  ...
  if (CPU_GPU) {
    AddGPUBurden_OP[oid] = sum / OCCount * scaler;
    printf("sum is %lf, CL_agg_min_afterGroupBy invocatio overhead in "
           "average in GPU, %lf\n",
           sum, AddGPUBurden_OP[oid]);
  } else {
    AddCPUBurden_OP[oid] = sum / OCCount * scaler;
...
void GROUP_BY_HandShake(int CPU_GPU) {
  ...
  if (CPU_GPU) {
    AddGPUBurden_OP[oid] = sum / OCCount * scaler;
    printf(
        "sum is %lf, CL_GroupBy invocatio overhead in average in GPU, %lf\n",
        sum, AddGPUBurden_OP[oid]);
  } else {
    AddCPUBurden_OP[oid] = sum / OCCount * scaler;
    printf(
        "sum is %lf, CL_GroupBy invocatio overhead in average in CPU, %lf\n",
        sum, AddCPUBurden_OP[oid]);
  }
}
void ORDER_BY_HandShake(int CPU_GPU) {
  ...
  if (CPU_GPU) {
    AddGPUBurden_OP[oid] = sum / OCCount * scaler;
    printf("sum is %lf, CL_RadixSort invocatio overhead in average in GPU, "
           "%lf\n",
           sum, AddGPUBurden_OP[oid]);
  } else {
    AddCPUBurden_OP[oid] = sum / OCCount * scaler;
    printf("sum is %lf, CL_RadixSort invocatio overhead in average in CPU, "
           "%lf\n",
           sum, AddCPUBurden_OP[oid]);
  }
}
void JOIN_NINLJ_HandShake(int CPU_GPU) {
  ...
  if (CPU_GPU) {
    AddGPUBurden_OP[oid] = sum / OCCount * scaler;
    printf("sum is %lf, CL_ninlj invocatio overhead in average in GPU, %lf\n",
           sum, AddGPUBurden_OP[oid]);
  } else {
    AddCPUBurden_OP[oid] = sum / OCCount * scaler;
    printf("sum is %lf, CL_ninlj invocatio overhead in average in CPU, %lf\n",
           sum, AddCPUBurden_OP[oid]);
  }
}
void JOIN_INLJ_HandShake(int CPU_GPU) {
  ...
  if (CPU_GPU) {
    AddGPUBurden_OP[oid] = sum / OCCount * scaler;
    printf("sum is %lf, CL_inlj invocatio overhead in average in GPU, %lf\n",
           sum, AddGPUBurden_OP[oid]);
  } else {
    AddCPUBurden_OP[oid] = sum / OCCount * scaler;
    printf("sum is %lf, CL_inlj invocatio overhead in average in CPU, %lf\n",
           sum, AddCPUBurden_OP[oid]);
  }
}
void JOIN_SMJ_HandShake(int CPU_GPU) {
  ...
  if (CPU_GPU) {
    AddGPUBurden_OP[oid] = sum / OCCount * scaler;
    printf("sum is %lf, CL_smj invocatio overhead in average in GPU, %lf\n",
           sum, AddGPUBurden_OP[oid]);
  } else {
    AddCPUBurden_OP[oid] = sum / OCCount * scaler;
    printf("sum is %lf, CL_smj invocatio overhead in average in CPU, %lf\n",
           sum, AddCPUBurden_OP[oid]);
  }
}
void JOIN_HJ_HandShake(int CPU_GPU) {
  ...
  if (CPU_GPU) {
    AddGPUBurden_OP[oid] = sum / OCCount * scaler;
    printf("sum is %lf, CL_hj invocatio overhead in average in GPU, %lf\n",
           sum, AddGPUBurden_OP[oid]);
  } else {
    AddCPUBurden_OP[oid] = sum / OCCount * scaler;
    printf("sum is %lf, CL_hj invocatio overhead in average in CPU, %lf\n",
           sum, AddCPUBurden_OP[oid]);
  }
}
void JOIN_MJ_HandShake(int CPU_GPU) {
  ...
  if (CPU_GPU) {
    AddGPUBurden_OP[oid] = sum / OCCount * scaler;
    printf("sum is %lf, CL_mj invocatio overhead in average in GPU, %lf\n",
           sum, AddGPUBurden_OP[oid]);
  } else {
    AddCPUBurden_OP[oid] = sum / OCCount * scaler;
    printf("sum is %lf, CL_mj invocatio overhead in average in CPU, %lf\n",
           sum, AddCPUBurden_OP[oid]);
  }
}
void HandShake() {
   ...
   * set speed threshold
   */
  // OP_LothresholdForGPUApp=_f(sortSpeedUp,0,counter-1,(int)(counter*20/58));//->set
  ...
  // OP_LothresholdForCPUApp=_f(sortSpeedUp,0,counter-1,(int)(counter*25/58+1));//->set
   ...
   */
  // OP_LoGPUBurden=_f(sortGPUBurden,0,counter-1,(int)(counter*40/58+1));//->set
  // lothreshold as first 40 max.
  OP_LoGPUBurden =
      (sortGPUBurden[0] + sortGPUBurden[(int)(counter * 10 / 58)]) / 2;
  printf("OP_LoGPUBurden is %lf\n", OP_LoGPUBurden);
  // OP_UpGPUBurden=_f(sortGPUBurden,0,counter-1,(int)(counter*10/58));//->set
  // UPthreshold as first 10 max.
  OP_UpGPUBurden = (sortGPUBurden[(int)(counter * 40 / 58 + 1)] +
   ...
   */
  // OP_LoCPUBurden=_f(sortCPUBurden,0,counter-1,(int)(counter*40/58+1));//->set
  // lothreshold as first 40 max.
  OP_LoCPUBurden =
      (sortCPUBurden[0] + sortCPUBurden[(int)(counter * 10 / 58)]) / 2;
  printf("OP_LoCPUBurden is %lf\n", OP_LoCPUBurden);
  // OP_UpCPUBurden=_f(sortCPUBurden,0,counter-1,(int)(counter*10/58));//->set
  // UPthreshold as first 10 max.
  OP_UpCPUBurden = (sortCPUBurden[(int)(counter * 40 / 58 + 1)] +
