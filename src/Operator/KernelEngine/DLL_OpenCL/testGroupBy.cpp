...
void groupByImpl_int(cl_mem d_R, int rLen, cl_mem d_groupLabel, int numThreadPB,
  ...
  kernel_enqueue(rLen, 9, 1, &globalWorkingSetSize, &numThreadsPerBlock_x,
                 eventList, index, kernel, Flag_CPU_GPU, burden, _CPU_GPU);
}
void groupByImpl_outSize_int(cl_mem d_outSize, cl_mem d_mark,
                             cl_mem d_markOutput, int rLen, int numThreadPB,
                             int numBlock, int *index, cl_event *eventList,
  ...
  kernel_enqueue(rLen, 2, 1, &globalWorkingSetSize, &numThreadsPerBlock_x,
                 eventList, index, kernel, Flag_CPU_GPU, burden, _CPU_GPU);
}
void groupByImpl_write_int(cl_mem d_startPos, cl_mem d_groupLabel,
                           cl_mem d_writePos, int rLen, int numThreadPB,
                           int numBlock, int *index, cl_event *eventList,
                           cl_kernel *kernel, int *Flag_CPU_GPU, double *burden,
                           int _CPU_GPU) {
  size_t numThreadsPerBlock_x = numThreadPB;
  ...
  // Set the Argument values
  cl_int ciErr1 =
      clSetKernelArg((*kernel), 0, sizeof(cl_mem), (void *)&d_startPos);
  ...
  kernel_enqueue(rLen, 8, 1, &globalWorkingSetSize, &numThreadsPerBlock_x,
                 eventList, index, kernel, Flag_CPU_GPU, burden, _CPU_GPU);
}
int groupByImpl(cl_mem d_Rin, int rLen, cl_mem d_Rout, cl_mem *d_startPos,
                int numThread, int numBlock, int *index, cl_event *eventList,
                cl_kernel *kernel, int *Flag_CPU_GPU, double *burden,
                int _CPU_GPU) {
  ...
  // sort
  cl_copyBuffer(d_Rout, d_Rin, memSize, index, eventList, Flag_CPU_GPU, burden,
                _CPU_GPU);
  radixSortImpl(d_Rout, rLen, 32, numThread, numBlock, index, eventList, kernel,
                Flag_CPU_GPU, burden, _CPU_GPU);
  CL_MALLOC(&d_groupLabel, sizeof(int) * rLen);
  groupByImpl_int(d_Rout, rLen, d_groupLabel, numThread, numBlock, index,
                  eventList, kernel, Flag_CPU_GPU, burden, _CPU_GPU);
  CL_MALLOC(&d_writePos, sizeof(int) * rLen);
  ScanPara *SP;
  SP = (ScanPara *)malloc(sizeof(ScanPara));
  initScan(rLen, SP);
  scanImpl(d_groupLabel, rLen, d_writePos, index, eventList, kernel,
           Flag_CPU_GPU, burden, SP, _CPU_GPU);
  CL_MALLOC(&d_numGroup, sizeof(int));
  groupByImpl_outSize_int(d_numGroup, d_groupLabel, d_writePos, rLen, 1, 1,
                          index, eventList, kernel, Flag_CPU_GPU, burden,
                          _CPU_GPU);
  ...
  groupByImpl_write_int((*d_startPos), d_groupLabel, d_writePos, rLen,
                        numThread, numBlock, index, eventList, kernel,
                        Flag_CPU_GPU, burden, _CPU_GPU);
  clWaitForEvents(1, &eventList[(*index - 1) % 2]);
  closeScan(SP);
...
void testGroupByImpl(int rLen, int numThread, int numBlock) {
  ...
  int numGroup = 0;
  // group by
  numGroup = CL_GroupBy((Record *)h_Rin, rLen, (Record *)h_Rout, &h_startPos,
                        numThread, numBlock, _CPU_GPU);
  // copy back
  validateGroupBy((Record *)h_Rin, rLen, (Record *)h_Rout, h_startPos,
                  numGroup);
