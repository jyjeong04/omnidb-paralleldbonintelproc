...
void filterImpl_map_int(cl_mem d_Rin, int beginPos, int rLen, cl_mem d_mark,
                        int smallKey, int largeKey, cl_mem d_temp,
                        int numThreadPB, int numBlock, int *index,
  ...
  kernel_enqueue(rLen, 20, 1, &globalWorkingSetSize, &numThreadsPerBlock_x,
                 eventList, index, Kernel, Flag_CPU_GPU, burden, _CPU_GPU);
}
void filterImpl_outSize_int(cl_mem d_outSize, cl_mem d_mark,
  ...
  kernel_enqueue(rLen, 21, 1, &globalWorkingSetSize, &numThreadsPerBlock_x,
                 eventList, index, Kernel, Flag_CPU_GPU, burden, _CPU_GPU);
}
void filterImpl_write_int(cl_mem d_Rout, cl_mem d_Rin, cl_mem d_mark,
  ...
  kernel_enqueue(rLen, 23, 1, &globalWorkingSetSize, &numThreadsPerBlock_x,
                 eventList, index, Kernel, Flag_CPU_GPU, burden, _CPU_GPU);
}

void filterImpl(cl_mem d_Rin, int beginPos, int rLen, cl_mem *d_Rout,
                int *outSize, int numThread, int numBlock, int smallKey,
                int largeKey, int *index, cl_event *eventList,
  ...
  filterImpl_map_int(d_Rin, beginPos, rLen, d_mark, smallKey, largeKey, d_temp,
  ...
  initScan(rLen, SP);
  scanImpl(d_mark, rLen, d_markOutput, index, eventList, Kernel, Flag_CPU_GPU,
           burden, SP, _CPU_GPU);
  clWaitForEvents(1, &eventList[(*index - 1) % 2]);
  closeScan(SP);
  ...
  filterImpl_outSize_int(d_outSize, d_mark, d_markOutput, rLen, 1, 1, index,
                         eventList, Kernel, Flag_CPU_GPU, burden, _CPU_GPU);
  clWaitForEvents(1, &eventList[(*index - 1) % 2]);
  // bufferchecking(d_outSize,sizeof(int));
  cl_readbuffer(outSize, d_outSize, sizeof(int), index, eventList, Flag_CPU_GPU,
                burden, _CPU_GPU);

  // write the reduced result
  CL_MALLOC(d_Rout, sizeof(Record) * (*outSize));
  filterImpl_write_int(*d_Rout, d_Rin, d_mark, d_markOutput, beginPos, rLen,
                       numThread, numBlock, index, eventList, Kernel,
                       Flag_CPU_GPU, burden, _CPU_GPU);
  clWaitForEvents(1, &eventList[(*index - 1) % 2]);
  CL_FREE(d_mark);
...
void testFilterImpl(int rLen, int numThreadPB,
                    int numBlock) //->corresponding to selection
{
  int _CPU_GPU = 0;
  /////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  CL_RangeSelection((Record *)Rin, rLen, smallKey, largeKey, &Rout, numThreadPB,
                    numBlock, _CPU_GPU);
  printf("CL_RangeSelectionFinish\n");
  CL_PointSelection((Record *)Rin, rLen, smallKey, &Rout, numThreadPB, numBlock,
                    _CPU_GPU);
