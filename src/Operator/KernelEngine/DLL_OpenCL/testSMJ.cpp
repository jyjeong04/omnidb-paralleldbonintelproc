...
void getQuantile(cl_mem d_R, int rLen, int interval, cl_mem d_output,
                 int numQuantile, int *index, cl_event *eventList,
  ...
  size_t thread[2] = {static_cast<size_t>(numThreadsPerBlock_x),
                      static_cast<size_t>(numThreadsPerBlock_y)};
  size_t grid[2] = {static_cast<size_t>(numBlock_X * numThreadsPerBlock_x *
                                        numBlock_Y * numThreadsPerBlock_y),
                    1};
  // quanMap_kernel<<<grid,thread, interval*sizeof(Record)>>>(d_R,
  // interval,rLen, d_output);
  ...
  kernel_enqueue(rLen, 44, 1, grid, thread, eventList, index, kernel,
                 Flag_CPU_GPU, burden, _CPU_GPU);
}

void joinMBCount(cl_mem d_R, int rLen, cl_mem d_S, int sLen, cl_mem d_quanLocS,
                 int numQuan, cl_mem d_n, dim3 grid_NLJ, dim3 threads_NLJ,
  ...
  kernel_enqueue(rLen, 46, 1, &globalWorkingSetSize, &numThreadsPerBlock_x,
                 eventList, index, kernel, Flag_CPU_GPU, burden, _CPU_GPU);
}

void joinMBWrite(cl_mem d_R, int rLen, cl_mem d_S, int sLen, cl_mem d_quanLocS,
                 int numQuan, cl_mem d_sum, cl_mem d_outBuf, dim3 grid_NLJ,
                 dim3 threads_NLJ, int *index, cl_event *eventList,
                 cl_kernel *kernel, int *Flag_CPU_GPU, double *burden,
                 int _CPU_GPU) {
  size_t numThreadsPerBlock_x = threads_NLJ.x;
  size_t globalWorkingSetSize = grid_NLJ.x * grid_NLJ.y * threads_NLJ.x;
  ...
  // Set the Argument values
  // gpuNLJ_int_kernel(cl_mem d_temp, Record *d_R, Record *d_S, int sStart, int
  // rLen, int sLen, int *d_n)
  cl_int ciErr1 = clSetKernelArg((*kernel), 0, sizeof(cl_mem), (void *)&d_R);
  ciErr1 |= clSetKernelArg((*kernel), 1, sizeof(cl_int), (void *)&rLen);
  ciErr1 |= clSetKernelArg((*kernel), 2, sizeof(cl_mem), (void *)&d_S);
  ciErr1 |= clSetKernelArg((*kernel), 3, sizeof(cl_int), (void *)&sLen);
  ...
  kernel_enqueue(rLen, 47, 1, &globalWorkingSetSize, &numThreadsPerBlock_x,
                 eventList, index, kernel, Flag_CPU_GPU, burden, _CPU_GPU);
}

int joinMatchingBlocks(cl_mem d_R, int rLen, cl_mem d_S, int sLen,
                       cl_mem d_quanLocS, int numQuan, cl_mem *d_Rout,
                       int *index, cl_event *eventList, cl_kernel *kernel,
                       int *Flag_CPU_GPU, double *burden, int _CPU_GPU) {
  int numResults = 0;
  ...
  int h_n = 0;
  int h_sum = 0;
  joinMBCount(d_R, rLen, d_S, sLen, d_quanLocS, numQuan, d_n, grid_NLJ,
              threads_NLJ, index, eventList, kernel, Flag_CPU_GPU, burden,
              _CPU_GPU);
  ScanPara *SP;
  SP = (ScanPara *)malloc(sizeof(ScanPara));
  initScan(resultBuf, SP);
  scanImpl(d_n, resultBuf, d_sum, index, eventList, kernel, Flag_CPU_GPU,
           burden, SP, _CPU_GPU);
  clWaitForEvents(1, &eventList[(*index - 1) % 2]);
  closeScan(SP);
  cl_readbuffer((void *)&h_n, d_n, (resultBuf - 1) * sizeof(int), sizeof(int),
                index, eventList, Flag_CPU_GPU, burden, _CPU_GPU);
  cl_readbuffer((void *)&h_sum, d_sum, (resultBuf - 1) * sizeof(int),
                sizeof(int), index, eventList, Flag_CPU_GPU, burden, _CPU_GPU);
  clWaitForEvents(1, &eventList[(*index - 1) % 2]);
  ...
  if (numResults > 0) {
    ...
    *d_Rout = d_outBuf;
    joinMBWrite(d_R, rLen, d_S, sLen, d_quanLocS, numQuan, d_sum, d_outBuf,
                grid_NLJ, threads_NLJ, index, eventList, kernel, Flag_CPU_GPU,
                burden, _CPU_GPU);
  }
  clWaitForEvents(1, &eventList[(*index - 1) % 2]);
  CL_FREE(d_n);
...
int MJImpl(cl_mem d_Rin, int rLen, cl_mem d_Sin, int sLen, cl_mem *d_Joinout,
           int *index, cl_event *eventList, cl_kernel *kernel,
           int *Flag_CPU_GPU, double *burden, int _CPU_GPU) {
  int numResult = 0;
  ...
  getQuantile(d_Rin, rLen, interval, d_quanKeyR, numQuanR, index, eventList,
              kernel, Flag_CPU_GPU, burden, _CPU_GPU);

  cl_mem d_quanLocS;
  CL_MALLOC(&d_quanLocS, numQuanR * sizeof(int) * 2);
  CUDA_CSSTree *tree;
  gpu_constructCSSTreeImpl(d_Rin, rLen, &tree, index, eventList, kernel,
                           Flag_CPU_GPU, burden, _CPU_GPU);
  cuda_search_index_usingKeys(tree->data, tree->nDataNodes, tree->dir,
                              tree->nDirNodes, d_quanKeyR, d_quanLocS,
                              numQuanR * 2, index, eventList, kernel,
                              Flag_CPU_GPU, burden, _CPU_GPU);
  clWaitForEvents(1, &eventList[(*index - 1) % 2]);
  numResult = joinMatchingBlocks(d_Rin, rLen, d_Sin, sLen, d_quanLocS, numQuanR,
                                 d_Joinout, index, eventList, kernel,
                                 Flag_CPU_GPU, burden, _CPU_GPU);
  clWaitForEvents(1, &eventList[(*index - 1) % 2]);
  CL_FREE(d_quanKeyR);
  CL_FREE(d_quanLocS);
...
int SMJImpl(cl_mem d_R, int rLen, cl_mem d_S, int sLen, cl_mem *d_Joinout,
            int *index, cl_event *eventList, cl_kernel *kernel,
            int *Flag_CPU_GPU, double *burden, int _CPU_GPU) {
  int numResult = 0;
  ...
  int numThreadPB = 256;
  int numBlock = 64;
  radixSortImpl(d_R, rLen, 32, numThreadPB, numBlock, index, eventList, kernel,
                Flag_CPU_GPU, burden, _CPU_GPU);
  radixSortImpl(d_S, sLen, 32, numThreadPB, numBlock, index, eventList, kernel,
                Flag_CPU_GPU, burden, _CPU_GPU);
  numResult = MJImpl(d_R, rLen, d_S, sLen, d_Joinout, index, eventList, kernel,
                     Flag_CPU_GPU, burden, _CPU_GPU);
