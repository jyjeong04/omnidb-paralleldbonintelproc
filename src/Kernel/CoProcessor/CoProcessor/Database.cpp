...
int Database::check(char *rName) {
  ...
  if (nameIndex->Lookup(rName, &id) == true) {
    cout << "table name duplicated: Database::AddTable(char *name):" << rName
         << endl;
    exit(-1);
  }
...
/*int Database::createTreeIndex(char* tableName, char* columnName)
{
        ...
        {
                CL_BuildTreeIndex(tables[id], Query_rLen,
&(cpu_treeIndexes[id]),0); tPro[id].cpu_treeindex=1;//indexed!!!
                //free(Rin);
        }
  ...
  if (src != NULL) {
    ...
    while (!feof(src)) {
      ...
      if (len <= NAME_MAX_LENGTH)
        charBuf[len - 1] = '\0';
      if (strcmp(charBuf, "[Tables]") == 0) { // tables
        while (!feof(src)) {
...
#else
          Record *R = (Record *)malloc(sizeof(Record) * Query_rLen);
          generateRand(R, Uplimit, Query_rLen,
                       this->numTable); // make it adaptive...
#endif
          this->addTable(charBuf, R, Query_rLen);
          cout << "importing " << charBuf << ", size of, " << Query_rLen
               << endl;
        }
...
conFile); char charBuf[NAME_MAX_LENGTH]; char dFileName[NAME_MAX_LENGTH]; while
(!feof(src))
                        {
                                fgets(charBuf,NAME_MAX_LENGTH,src);
                                len=(int)strlen(charBuf);
                                ...
                                {//tables
                                        while(!feof(src))
                                        {
                                                fgets(charBuf,NAME_MAX_LENGTH,src);//It
is a table Name. len=(int)strlen(charBuf); if(len<=NAME_MAX_LENGTH)
charBuf[len-1]='\0'; if(strcmp(charBuf,"[/Tables]")==0) break;
                                                fgets(dFileName,NAME_MAX_LENGTH,src);
                                                len=(int)strlen(dFileName);
                                                if(len<=NAME_MAX_LENGTH)
dFileName[len-1]='\0'; FILE *dbFile = fopen(dFileName, "w"); if(dbFile!=NULL)
                                                {
                                                        Record** R=(Record
**)malloc(sizeof(Record*)); int Query_rLen=0; getTable(charBuf,R,&Query_rLen);
                                                        fprintf(dbFile,
"%d\n",Query_rLen); printf("%s, len, %d\n",dFileName,Query_rLen); for(int
i=0;i<Query_rLen;i++)
                                                        {
                                                                fprintf(dbFile,
"%d ",(*R)[i].rid); fprintf(dbFile, "%d\n",(*R)[i].value);
                                                        }
                                                        free(R);
                                                }
                                                fclose(dbFile);
...
int Database::dbmbenchForMonet(int scale) {
  ...
  // T2.a1, primary key, for testing, we just let is be 1...DEFAULT_DB_SIZE,
  // random.
  char dbFileName[120];
...
int Database::dbmbench(int scale) {
  ...
  cout << "total database size, "
       << sizeof(Record) * DEFAULT_DB_SIZE * 3 * (scale + 1) / 1024 / 1024
       << " MB," << endl;
  return 0;
  ...
  //T2.a1, primary key, for testing, we just let is be 1...DEFAULT_DB_SIZE,
random. FILE *dbFile = fopen("./dbmbench/T2.a1.dat", "w"); Record
*R=(Record*)malloc(sizeof(Record)*Query_rLen); if(dbFile!=NULL)
  {
          for(i=0;i<Query_rLen;i++)
          {
                  R[i].value=i;
