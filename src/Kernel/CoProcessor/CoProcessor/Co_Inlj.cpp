...
#else
void* tp_inlj(void* lpParam) {
#endif
  ws_coinlj *pData;
  pData = (ws_coinlj *)lpParam;
  Record *R = pData->R;
  int rLen = pData->Query_rLen;
  Record *S = pData->S;
  int sLen = pData->sLen;
  ...
  while (1) {
    ...
    *(pData->curPosToJoin) = toPos;
    ...
    if (execMode == EXEC_CPU) {
      ...
      clock_t tt = 0;
      startTimer(tt);
      numResult = CPU_inlj(R, rLen, cpu_tree, S + fromPos, realBlockSize,
                           &tempRout, OMP_JOIN_NUM_THREAD);
    ...
    } else // on the GPU
    {
      ...
      clock_t tt = 0;
      startTimer(tt);
      numResult = GPUCopy_inlj(R, rLen, gpu_tree, S + fromPos, realBlockSize,
                               &tempRout);
...
/*
we first cut on the larger relation assuming the smaller one fits into the
device memory. Otherwise, we further cut the smaller one into multiple chunks.
(not implemented).
*/

int CO_inlj(Record *R, int rLen, CC_CSSTree *cT, CUDA_CSSTree *gT, Record *S,
            int sLen, Record **Rout) {
  int resultSize = 0;
  ...
  for (i = 0; i < numThread; i++) {
    ...
    pData[i] = (ws_coinlj *)calloc(1, sizeof(ws_coinlj));
    ...
    if (i == 0) // GPU first.
    {
      pData[i]->init(R, rLen, gpuBlockSize, S, sLen, cT, gT, dispatchMutex,
                     mergeMutex, &curP, EXEC_GPU, tempResultVec, tempSizeVec);
    } else {
      pData[i]->init(R, rLen, cpuBlockSize, S, sLen, cT, gT, dispatchMutex,
                     mergeMutex, &curP, EXEC_CPU, tempResultVec, tempSizeVec);
    }
