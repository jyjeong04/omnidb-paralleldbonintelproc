...
/*	void perscan_kernel( cl_mem d_odata, cl_mem d_idata, int
 * numElementsPerBlock, bool isFull, int base, int d_odataOffset, int OPERATOR
 * )*/
void perscan_kernel_int(cl_mem temp, cl_mem d_odata, cl_mem d_idata,
                        int numElementsPerBlock, bool isFull, int base,
                        int d_odataOffset, int OPERATOR, int numBlock,
                        int numThread, int sharedMemSize, int rLen, int *index,
                        cl_event *eventList, cl_kernel *kernel,
  ...
  ciErr1 |=
      clSetKernelArg((*kernel), 6, sizeof(cl_int), (void *)&d_odataOffset);
  ciErr1 |= clSetKernelArg((*kernel), 7, sizeof(cl_int), (void *)&OPERATOR);
  ciErr1 |=
      clSetKernelArg((*kernel), 8, sizeof(cl_int), (void *)&sharedMemSize);
  ...
  kernel_enqueue(1, 7 // seems not related to size
                 ,
                 1, &globalWorkingSetSize, &numThreadsPerBlock_x, eventList,
                 index, kernel, Flag_CPU_GPU, burden, _CPU_GPU);
}
void getResult_kernel_init(cl_mem d_Result, cl_mem d_Rout, int rLen,
                           int OPERATOR, int numThread, int numBlock,
                           int *index, cl_event *eventList, cl_kernel *kernel,
                           int *Flag_CPU_GPU, double *burden, int _CPU_GPU)
// cl_mem d_Result, cl_mem d_Rout, int rLen, int OPERATOR
{
  ...
  // Set the Argument values
  cl_int ciErr1 =
      clSetKernelArg((*kernel), 0, sizeof(cl_mem), (void *)&d_Result);
  ...
  kernel_enqueue(rLen, 1, 1, &globalWorkingSetSize, &numThreadsPerBlock_x,
                 eventList, index, kernel, Flag_CPU_GPU, burden, _CPU_GPU);
}
int reduceBlockSums(cl_mem d_Rout, int maxNumThread, int OPERATOR, int rLen,
                    int *index, cl_event *eventList, cl_kernel *kernel,
                    int *Flag_CPU_GPU, double *burden, tempResult *tR,
                    int _CPU_GPU) {
  ...
  // if no, info[0]: number of blocks except of the last block, info[1]: number
  // of thread in the last block
  for (int level = 0; level < (tR->d_numLevelsAllocated - 1); level++) {
    ...
    if (numBlock > 0) {
      ...
      perscan_kernel_int(temp, tR->d_scanBlockSums[level + 1],
                         tR->d_scanBlockSums[level], numElementsPerBlock, true,
                         0, 0, OPERATOR, numBlock, maxNumThread, sharedMemSize,
                         rLen, index, eventList, kernel, Flag_CPU_GPU, burden,
                         _CPU_GPU);
    ...
    if ((!isMul) || (numBlock == 0)) {
      ...
      unsigned int remainer =
          tR->levelSize[level] - numElementsPerBlock * info[0];
      ...
      if (numThread == 0) {
        cl_copyBuffer((tR->d_scanBlockSums[level + 1]), offset,
                      tR->d_scanBlockSums[level], base, sizeof(int), index,
                      eventList, Flag_CPU_GPU, burden, _CPU_GPU);
        ...
        if (isPowerOfTwo(remainer)) {
          perscan_kernel_int(temp, tR->d_scanBlockSums[level + 1],
                             tR->d_scanBlockSums[level], remainer, true, base,
                             offset, OPERATOR, numBlock, numThread,
                             sharedMemSize, rLen, index, eventList, kernel,
                             Flag_CPU_GPU, burden, _CPU_GPU);
          clWaitForEvents(1, &eventList[(*index - 1) % 2]);
        } else {

          perscan_kernel_int(temp, tR->d_scanBlockSums[level + 1],
                             tR->d_scanBlockSums[level], remainer, false, base,
                             offset, OPERATOR, numBlock, numThread,
                             sharedMemSize, rLen, index, eventList, kernel,
                             Flag_CPU_GPU, burden, _CPU_GPU);
          clWaitForEvents(1, &eventList[(*index - 1) % 2]);
  ...
  clWaitForEvents(1, &eventList[(*index - 1) % 2]);
  getResult_kernel_init(tR->d_scanBlockSums[tR->d_numLevelsAllocated - 1],
                        d_Rout, rLen, OPERATOR, 1, 1, index, eventList, kernel,
                        Flag_CPU_GPU, burden, _CPU_GPU);
  clWaitForEvents(1, &eventList[(*index - 1) % 2]);
  CL_FREE(temp);
  return 1;
...
void perscanFirstPass_kernel_int(cl_mem t_temp, cl_mem d_temp, cl_mem d_odata,
                                 cl_mem d_idata, int numElementsPerBlock,
                                 bool isFull, int base, int d_odataOffset,
                                 int OPERATOR, int numBlock, int numThread,
                                 unsigned int sharedMemSize, int rLen,
                                 int *index, cl_event *eventList,
                                 cl_kernel *kernel, int *Flag_CPU_GPU,
  ...
  kernel_enqueue(1, 6, 1, &globalWorkingSetSize, &numThreadsPerBlock_x,
                 eventList, index, kernel, Flag_CPU_GPU, burden, _CPU_GPU);
}
void copyLastElement_kernel_int(cl_mem d_Rout, cl_mem d_Rin, int base,
                                int offset, int numBlock, int numThread,
                                int *index, cl_event *eventList,
                                cl_kernel *kernel, int *Flag_CPU_GPU,
                                double *burden, int _CPU_GPU) {
  size_t numThreadsPerBlock_x = numThread;
  ...
  kernel_enqueue(1, 5, 1, &globalWorkingSetSize, &numThreadsPerBlock_x,
                 eventList, index, kernel, Flag_CPU_GPU, burden, _CPU_GPU);
}
void reduceFirstPass(cl_mem d_Rin, int rLen, int numThread, int numMaxBlock,
                     int OPERATOR, int *index, cl_event *eventList,
                     cl_kernel *kernel, int *Flag_CPU_GPU, double *burden,
                     tempResult *tR, int _CPU_GPU) {
  int *info = (int *)malloc(sizeof(int) * 2);
  // get the information of partition
  // return bool: if is multiple of maxNumThread
  // if yes, info[0]: number of blocks, info[1] = maxNumThread
  ...
  if (numBlock > 0) {
    ...
    for (int chunkIdx = 0; chunkIdx < numChunk; chunkIdx++) {
      ...
      int subNumBlock = (chunkIdx == (numChunk - 1))
                            ? (numBlock - chunkIdx * numMaxBlock)
                            : (numMaxBlock);

      numElementsPerBlock = numThread * 2;
      extraSpace = numElementsPerBlock / NUM_BANKS;
      sharedMemSize = sizeof(int) * (numElementsPerBlock + extraSpace);
      perscanFirstPass_kernel_int(
          t_temp, d_temp, tR->d_scanBlockSums[0], d_Rin, numElementsPerBlock,
          true, base, offset, OPERATOR, subNumBlock, numThread, sharedMemSize,
          rLen, index, eventList, kernel, Flag_CPU_GPU, burden, _CPU_GPU);
      clWaitForEvents(1, &eventList[(*index - 1) % 2]);
    }
  }
  clWaitForEvents(1, &eventList[(*index - 1) % 2]);
  ...
  if ((!isMul) || (numBlock == 0)) {
    ...
    if (numThread == 0) {
      copyLastElement_kernel_int(tR->d_scanBlockSums[0], d_Rin, base, offset, 1,
                                 1, index, eventList, kernel, Flag_CPU_GPU,
                                 burden, _CPU_GPU);
      clWaitForEvents(1, &eventList[(*index - 1) % 2]);
    } else {
      numBlock = 1;
      ...
      if (isPowerOfTwo(remainer)) {
        ...
        perscanFirstPass_kernel_int(
            t_temp, d_temp, tR->d_scanBlockSums[0], d_Rin, remainer, true, base,
            offset, OPERATOR, numBlock, numThread, sharedMemSize, rLen, index,
            eventList, kernel, Flag_CPU_GPU, burden, _CPU_GPU);
        clWaitForEvents(1, &eventList[(*index - 1) % 2]);
      } else {
        perscanFirstPass_kernel_int(
            t_temp, d_temp, tR->d_scanBlockSums[0], d_Rin, remainer, false,
            base, offset, OPERATOR, numBlock, numThread, sharedMemSize, rLen,
            index, eventList, kernel, Flag_CPU_GPU, burden, _CPU_GPU);
        clWaitForEvents(1, &eventList[(*index - 1) % 2]);
      }
    }
  }
...
int reduceImpl(cl_mem d_Rin, int rLen, cl_mem d_Rout, int OPERATOR,
               int numThread, int numMaxBlock, int *index, cl_event *eventList,
               cl_kernel *kernel, int *Flag_CPU_GPU, double *burden,
               tempResult *tR, int _CPU_GPU) {
  preallocBlockSums(rLen, numThread, tR);
  reduceFirstPass(d_Rin, rLen, numThread, numMaxBlock, OPERATOR, index,
                  eventList, kernel, Flag_CPU_GPU, burden, tR, _CPU_GPU);
  int result =
      reduceBlockSums(d_Rout, numThread, OPERATOR, rLen, index, eventList,
                      kernel, Flag_CPU_GPU, burden, tR, _CPU_GPU);
  clWaitForEvents(1, &eventList[(*index - 1) % 2]);
  return result;
}
// return bool: if is multiple of maxNumThread
// if yes, info[0]: number of blocks, info[1] = maxNumThread
// if no, info[0]: number of blocks except of the last block, info[1]: number of
// thread in the last block
void testReduceImpl(int rLen, int OPERATOR, int numThreadPB, int numMaxBlock) {
  ...
  numResult =
      CL_AggMaxOnly(d_Rin, rLen, &d_Rout, numThreadPB, numMaxBlock, _CPU_GPU);
