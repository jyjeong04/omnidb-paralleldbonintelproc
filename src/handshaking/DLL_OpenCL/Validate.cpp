...
void validateScan(int *input, int rLen, int *output) {
  ...
  for (int i = 0; i < rLen; i++) {
    ...
    // printf("checkOutput[%d ]=%d ,
    // output[%d]=%d",i,checkOutput[i],i,output[i]);
...
void validateProjection(Record *h_Rin, int rLen, Record *originalProjTable,
                        Record *h_projTable, int pLen) {
  ...
  for (int i = 0; i < pLen; i++) {
    if ((h_projTable[i].x != checkProjTable[i].x) ||
        (h_projTable[i].y != h_projTable[i].y)) {
      pass = false;
      break;
    }
  }
...
void validateAggAfterGroupBy(Record *h_Rin, int rLen, int *startPos,
                             int numGroups, Record *Ragg, int *aggResults,
                             int OPERATOR) {
  bool pass = true;
...
void validateGroupBy(Record *h_Rin, int rLen, Record *h_Rout, int *h_startPos,
                     int numGroup) {
  bool pass = true;
...
void validateFilter(Record *h_Rin, int beginPos, int rLen, Record *h_Rout,
                    int outSize, int smallKey, int largeKey) {
  bool passed = true;
...
void validateReduce(Record *h_Rin, int rLen, unsigned int gpuResult,
                    int OPERATOR) {
  int timer = DLL_genTimer(0);
...
/*void gpuValidateSort(Record *d_R, int rLen)
{
        ...
        for(i=1;i<rLen;i++)
        {
                ...
                {
                        printf("!!!!!!!! error in sorting: %d, %d, %d, %d\n",
i-1, R[i-1].y, i,h_Rin[i].y); passed=false; return;
                }
        }
        if(passed)
                printf("sorting passed\n");
