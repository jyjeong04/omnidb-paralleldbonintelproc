...
inline unsigned int RSHash(int value, int mask) {
...
// hash join
#define _maxPartLen                                                            \
  (512) // max partition length. Limited by shared memory size (4k to be safe):
        // sizeof(Rec) * maxPartLen <= 4k
#define HASH(v)                                                                \
  RSHash(v,                                                                    \
         ((1 << 30) -                                                          \
          1)) //(((unsigned int)( (v >> 7) ^ (v >> 13) ^ (v >>21) ^ (v) )) )
...
// new added part.
__kernel // kid=0
    void projection_map_kernel(__global Record *d_projTable, int pLen,
                               __global int *d_loc, __global int *d_temp) {
...
__kernel // kid=1
    void getResult_kernel(__global int *d_Result, __global Record *d_Rout,
                          int rLen, int OPERATOR) {
  d_Rout[0].x = 0;
...
__kernel void // kid=2
groupByImpl_outSize_kernel(__global int *d_outSize, __global int *d_mark,
                           __global int *d_markOutput, int rLen) {
  *d_outSize = d_mark[rLen - 1] + d_markOutput[rLen - 1];
}
__kernel // kid=3
    void mapBeforeGather_kernel(__global Record *d_Rin, int rLen,
                                __global int *d_loc, __global int *d_temp) {
  int bx = get_group_id(0);
  int tx = get_local_id(0);
  int numThread = get_local_size(0);
...
__kernel // kid=4
    void parallelAggregate_kernel(__global Record *d_S,
                                  __global int *d_startPos,
                                  __global int *d_aggResults, int OPERATOR,
                                  int blockOffset, int numGroups, int rLen,
                                  __local int *s_data) {
...
__kernel // kid=5
    void copyLastElement_kernel(__global int *d_odata, __global Record *d_Rin,
                                int base, int offset) {
  d_odata[offset] = d_Rin[base].y;
}
// with shared memory, with coalesced
__kernel // kid=6
    void perscanFirstPass_kernel(__global int *temp, __global int *d_temp,
                                 __global int *d_odata,
                                 __global Record *d_idata,
                                 int numElementsPerBlock, int isFull, int base,
                                 int d_odataOffset, int OPERATOR,
                                 int sharedMemSize) {
  {
    ...
    if (OPERATOR == REDUCE_SUM || OPERATOR == REDUCE_AVERAGE) {
      if (isFull) {
        ...
        temp[bi + bankOffsetB] =
            (bi < numElementsPerBlock) ? (d_idata[mem_bi].y) : (0);
      }
      ...
      if (isFull) {
        ...
        temp[bi + bankOffsetB] =
            (bi < numElementsPerBlock) ? (d_idata[mem_bi].y) : (TEST_MIN);
      }
    } else if (OPERATOR == REDUCE_MIN) {
      if (isFull) {
        d_temp[mem_bi] = d_idata[mem_bi].x;
        ...
        temp[bi + bankOffsetB] =
            (bi < numElementsPerBlock) ? (d_idata[mem_bi].y) : (TEST_MAX);
      }
    }
...
__kernel // kid=7
    void perscan_kernel(__global int *d_odata, __global int *temp,
                        __global int *d_idata, int numElementsPerBlock,
                        int isFull, int base, int d_odataOffset, int OPERATOR,
  ...
  if (OPERATOR == REDUCE_SUM || OPERATOR == REDUCE_AVERAGE) {
    if (isFull) {
      ...
      temp[bi + bankOffsetB] =
          (bi < numElementsPerBlock) ? (d_idata[mem_bi]) : (0);
    }
  } else if (OPERATOR == REDUCE_MAX) {
    if (isFull) {
      temp[bi + bankOffsetB] = d_idata[mem_bi];
      ...
      temp[bi + bankOffsetB] =
          (bi < numElementsPerBlock) ? (d_idata[mem_bi]) : (TEST_MIN);
    }
  } else if (OPERATOR == REDUCE_MIN) {
    if (isFull) {
      temp[bi + bankOffsetB] = d_idata[mem_bi];
      ...
      temp[bi + bankOffsetB] =
          (bi < numElementsPerBlock) ? (d_idata[mem_bi]) : (TEST_MAX);
    }
  }
...
__kernel // kid=8
    void groupByImpl_write_kernel(__global int *d_startPos,
                                  __global int *d_groupLabel,
                                  __global int *d_writePos, int rLen) {
  int bx = get_group_id(0);
  int tx = get_local_id(0);
  const int blockDimX = get_local_size(0);
  const int gridDimX = get_num_groups(0);
...
__kernel // kid=9
    void scanGroupLabel_kernel(__global Record *d_Rin, int rLen,
                               __global int *d_groupLabel) {
  int bx = get_group_id(0);
  int tx = get_local_id(0);
  const int blockDimX = get_local_size(0);
...
setRIDList_kernel(__global int *d_RIDList, int delta, __global int *d_intput,
                  int rLen, __global Record *d_R) {
...
getRIDList_kernel(__global int *d_RIDList, int delta, __global int *d_output,
                  int rLen, __global Record *d_Rin) {
  const int by = get_group_id(1);
  const int bx = get_group_id(0);
  const int tx = get_local_id(0);
  const int ty = get_local_id(1);
...
setValueList_kernel(__global int *d_ValueList, int delta, int rLen,
                    __global Record *d_R) {
  const int by = get_group_id(1);
  const int bx = get_group_id(0);
  const int tx = get_local_id(0);
...
getValueList_kernel(__global int *d_ValueList, int delta,
                    __global int *d_output, int rLen, __global Record *d_Rin) {
  const int by = get_group_id(1);
  const int bx = get_group_id(0);
  const int tx = get_local_id(0);
  const int ty = get_local_id(1);
...
mapImpl_kernel(__global Record *d_R, int rLen, __global int *d_output1,
               __global int *d_output2) {
...
mapImplVec_kernel(__global uint4 *d_R, int rLen, __global uint2 *d_output1,
                  __global uint2 *d_output2) {
  int iGID = get_global_id(0);
  uint4 value;
  int delta = get_global_size(0);
  uint2 first;
...
__kernel // kid=19
    void ScanLargeArrays_kernel(__global int *output, __global int *input,
...
filterImpl_map_kernel(__global Record *d_Rin, int beginPos, int rLen,
                      __global int *d_mark, int smallKey, int largeKey,
                      __global int *d_temp, __global WASEntry *was_buffer,
...
filterImpl_map_noCoalesced_kernel(__global Record* d_Rin, int beginPos, int
rLen, __global int* d_mark, int smallKey, int largeKey, __global int* d_temp )
{
        int delta=get_global_size(0);
...
__kernel void // kid 21
filterImpl_outSize_kernel(__global int *d_outSize, __global int *d_mark,
                          __global int *d_markOutput, int rLen) {
  *d_outSize = d_mark[rLen - 1] + d_markOutput[rLen - 1];
}

__kernel void // kid=22
filterImpl_write_noCoalesced_kernel(__global Record *d_Rout,
                                    __global Record *d_Rin,
                                    __global int *d_mark,
                                    __global int *d_markOutput, int beginPos,
                                    int rLen) {
  int delta = get_global_size(0);
  unsigned int chunkSize = rLen / delta;
  const int resultID = get_global_id(0);
...
filterImpl_write_kernel(__global Record *d_Rout, __global Record *d_Rin,
                        __global int *d_mark, __global int *d_markOutput,
                        int beginPos, int rLen) {
  int iGID = get_global_id(0);
  Record value;
  int delta = get_global_size(0);
  int flag = 0;
...
optScatter_kernel(__global Record *d_R, int rLen, __global int *loc, int from,
                  int to, __global Record *d_S) {
  int iGID = get_global_id(0);
  int delta = get_global_size(0);
  int targetLoc = 0;
  for (int pos = iGID; pos < rLen; pos += delta) {
...
optGather_kernel(__global Record *d_R, int rLen, __global int *loc, int from,
                 int to, __global Record *d_S, int sLen) {
  int iGID = get_global_id(0);
  int delta = get_global_size(0);
  int targetLoc = 0;
...
inline int getPartID(int key, int numPart) { return (key % numPart); }

// for split
__kernel // kid=26
    void partition_kernel(__global Record *d_R, int rLen, int numPart,
                          __global int *d_output1, __global int *d_output2) {
  int iGID = get_global_id(0);
  int delta = get_global_size(0);
  int targetLoc = 0;
  for (int pos = iGID; pos < rLen; pos += delta) {
...
mapPart_kernel(__global Record *d_R, int rLen, int numPart,
               __global int *d_output1, __global int *d_output2) {
  int iGID = get_global_id(0);
  int delta = get_global_size(0);
  int targetLoc = 0;
...
// d_hist layout: d_hist[thread global ID + total number threads*partition ID]
// is the count.
__kernel // kid=28
...
__kernel // kid=29
    void writeHist_kernel(__global int *d_pidArray, int rLen, int numPart,
                          __global int *d_psSum, __global int *d_loc,
                          __local int *shared_hist) {
  // extern __shared__ int shared_hist[];
  const int tid = get_local_id(0);
  const int bid = get_group_id(0);
  const int resultID = get_global_id(0);
...
getBound_kernel(__global int *d_psSum, int interval, int rLen, int numPart,
                __global Record *d_bound) {
  const int tid = get_local_id(0);
  const int bid = get_group_id(0);
  const int resultID = get_global_id(0);
  if (resultID < numPart) {
...
__kernel // kid 31
    void BitonicSort_kernel(__global Record *theArray, const uint stage,
...
__kernel // kid=38
    void gpuNLJ_kernel(__global int *d_temp, __global Record *d_R,
                       __global Record *d_S, int sStart, int rLen, int sLen,
                       __global int *d_n) {
...
nlj_write_kernel(__global Record *d_R, __global Record *d_S, int sStart,
                 int rLen, int sLen, __global int *d_sum,
                 __global Record *output) {
  __local Record shared_s[NLJ_S_BLOCK_SIZE];
  int bid = get_group_id(0) + get_group_id(1) * get_num_groups(0);
  int tid = get_local_id(0) + get_local_id(1) * get_local_size(0);
  int resultID = (bid)*NLJ_NUM_THREADS_PER_BLOCK + tid;
...
int getRightMostDescIdx(int tree_size, int nodeIdx) {
  ...
  int n = uintCeilingLog(
              TREE_FANOUT,
              uintCeilingDiv(TREE_NODE_SIZE * tree_size + TREE_FANOUT, tmp)) -
          1;
...
__kernel // kid=40
    void gSearchTree_kernel(__global IDataNode *data, int nDataNodes,
                            __global IDirectoryNode *dir, int nDirNodes,
                            int lvlDir, __global Record *arr,
                            __global int *locations, int nSearchKeys,
...
__kernel // kid=41
    void gIndexJoin_kernel(__global Record *R, int rLen, __global Record *S,
                           __global int *g_locations, int sLen,
                           __global int *g_ResNums, int clusterSize) {
  int bid = get_group_id(0);
  int tid = get_local_id(0);
...
__kernel // kid=42
    void gJoinWithWrite_kernel(__global Record *R, int rLen, __global Record *S,
                               __global int *g_locations, int sLen,
                               __global int *g_PrefixSums,
                               __global Record *g_joinResultBuffers,
                               int clusterSize) {
  int bid = get_group_id(0);
  int tid = get_local_id(0);
  int cluster_id = bid * THRD_PER_BLCK_join + tid;
...
__kernel // kid=43
    void gCreateIndex_kernel(__global IDataNode *data,
                             __global IDirectoryNode *dir, int dirSize,
  ...
  for (int nodeIdx = startIdx; nodeIdx < endIdx; nodeIdx++) {
    int childIdx =
        nodeIdx * TREE_FANOUT + keyIdx + 1; // One step down to the left
    ...
    // versus the unusual case when the tree is incomplete and the node does not
    // have the full set of children
    else {
    ...
    int dataArrayIdx =
        getDataArrayIdx(dirSize, tree_size, bottom_start, rightMostDesIdx);

    dir[nodeIdx].keys[keyIdx] =
        data[dataArrayIdx].records[TREE_NODE_SIZE - 1].y;
  }
}
__kernel // kid=44
    ...
    for (pos = sStart; (pos + tid) < sEnd; pos += SMJ_NUM_THREADS_PER_BLOCK) {
      ...
      for (i = startPos; i < SMJ_NUM_THREADS_PER_BLOCK; i++)
        if (tempBuf_R[i].y == tempValue.y)
          numResult++;
        else if (tempBuf_R[i].y > tempValue.y)
          break;
    }
  } else
    numResult = 0;
...
__kernel // kid=48
    void Histo_kernel(__global int *d_HistoMat, __global Record *d_R,
                      const int nR, const int pn, const int shift,
                      __local int *s_histo) {
  const int gridDimX = get_num_groups(0);
  const int blockDimX = get_local_size(0);
...
__kernel // kid=49
    void Reorder_kernel(__global Record *d_R1, __global int *d_PBound,
                        __global Record *d_R, __global int *d_WriteLoc,
                        const int nR, const int pn, const int shift,
                        __local int *s_writeLoc) {
  const int gridDimX = get_num_groups(0);
  const int blockDimX = get_local_size(0);
  const int bidX = get_group_id(0);
  const int tidX = get_local_id(0);
  ...
  if (offset ==
      0) // this thread's corresponding HistMatPre contains d_partbound2
  {
    for (int pi = 0; pi < pn; ++pi) {
...
//		p=d_PID[r]=hash(r); histo[p]++; sync; d_PidHisto[snakewise
// p]=histo;
__kernel // kid=50
    void Histo3_kernel(__global int *d_HistoMat, __global Record *d_R,
                       __global int *d_PBound, const int nParent, const int pn,
                       const int shift, __local int *s_histo) {
  const int gridDimX = get_num_groups(0);
  const int blockDimX = get_local_size(0);
  const int bidX = get_group_id(0);
  const int tidX = get_local_id(0);
...
__kernel // kid=51
    void Reorder3_kernel(__global Record *d_R1, __global int *d_RDir1,
                         __global Record *d_R, __global int *d_RDir,
                         __global int *d_Loc, const int nParent, const int pn,
  ...
  if ((bx == bx0) &&
      (tx == 0)) // this writeLoc is also the child partition's start
  {
    for (int pi = 0; pi < pn; ++pi) {
...
__kernel // kid=52
  ...
  if ((r2 - r1 > _maxPartLen) &&
      (s2 - s1 > _maxPartLen)) // overflow shared memory
    return;
  ...
  for (int k = 2; k <= blockDimX; k *= 2) {
    ...
    for (int j = k / 2; j > 0; j /= 2) {
      int ixj = tid ^ j;
      if (ixj > tid) // should carry the INTMAX together, o/w will err!:  if
                     // ((ixj > tid) && (tid < partLen))
      {
        if ((tid & k) == 0) {
...
__kernel // kid=53
    void Probe_Cnt_kernel(__global int *d_ThreadCnts, __global int *d_skewPid,
                          __global Record *d_R, __global int *d_PBoundR,
                          __global Record *d_S, __global int *d_PBoundS,
  ...
  while (px < PnG) {
    ...
    } else if ((r2 - r1) <= (s2 - s1)) // read R, scan S
    {
      ...
      for (int i = 0; offset < s2; ++i) {
        tmpRec = d_S[offset];
        /*//#ifdef _bSortPart
                                        firstHit = FirstHit(s_Table, r2 - r1,
        tmpRec.y); if(firstHit >= 0)
                                        {
                                                do{
                                                        ++count;
                                                        ++firstHit;
                                                }while(s_Table[firstHit].y ==
        tmpRec.y);
                                        }
        #else*/
    ...
    } else //((r2 - r1) >= (s2 - s1))	//read S, scan R
    {
      ...
      for (int i = 0; offset < r2; ++i) {
        tmpRec = d_R[offset];
        /*//#ifdef _bSortPart
                                        firstHit = FirstHit(s_Table, s2 - s1,
        tmpRec.y); if(firstHit >= 0)
                                        {
                                                do{
                                                        ++count;
...
// use s to probe r, write to global positions that d_ThreadCntsPresum
// indicates
__kernel // kid=54
    void Probe_Write_kernel(__global Record *d_RS,
                            __global int *d_ThreadCntsPresum,
                            __global Record *d_R, __global int *d_PBoundR,
                            __global Record *d_S, __global int *d_PBoundS,
                            const int PnG, __local Record *s_Table) {
  // extern __shared__ Record s_Table[];
  ...
  const int tidX = get_local_id(0);
  int outputPos =
      d_ThreadCntsPresum[bidX * blockDimX + tidX]; // output's starting offset
  int px = bidX;                                   // part idx
  Record tmpRec;
  Record tmpRS;
  int r1, r2, s1, s2;
        ...
        tmpRec.y); if(firstHit >= 0)
                                        {
                                                do{
                                                        tmpRS.x =
        s_Table[firstHit].x;	//x tmpRS.y = tmpRec.x;	//sid d_RS[outputPos] =
        tmpRS;
                                                        ++outputPos;
                                                        ++firstHit;
                                                }
        ...
        tmpRec.y); if(firstHit >= 0)
                                        {
                                                do{
                                                        tmpRS.x = tmpRec.x;
        //x tmpRS.y = s_Table[firstHit].x;	//sid d_RS[outputPos] = tmpRS;
                                                        ++outputPos;
                                                        ++firstHit;
                                                }
                                                while(s_Table[firstHit].y ==
...
// the overflowed partition falls naturally into some fragments, each length
// parLen/shareMemsize. then each block handles one frag in strip-mining way.
__kernel // kid=55
    void Probe_CntOverflow_kernel(__global int *d_ThreadCnts, const int skewPid,
                                  __global Record *d_R, __global int *d_PBoundR,
                                  __global Record *d_S, __global int *d_PBoundS,
  ...
  if ((rEnd - rStart) <= (sEnd - sStart)) {
    ...
    while (FIdx < nFrag) {
      ...
      int fragEnd =
          (fragStart + _maxPartLen < rEnd) ? (fragStart + _maxPartLen) : rEnd;
  ...
  } else //(rEnd - rStart) >= (sEnd - sStart)), use s as inner table
  {
    ...
    while (FIdx < nFrag) {
      ...
      int fragEnd =
          (fragStart + _maxPartLen < sEnd) ? (fragStart + _maxPartLen) : sEnd;
      int offset = fragStart + tidX; // global
      for (int i = 0; offset < fragEnd; ++i) {
        s_Table[tidX + i * blockDimX] = d_S[offset];
        offset = offset + blockDimX;
...
// parLen/shareMemsize. then each block handles one frag in strip-mining way.
__kernel // kid=56
    void Probe_WriteOverflow_kernel(
        __global Record *d_RS, __global int *d_WriteLoc, const int skewPid,
...
uint sim_hash(uint key, uint mod) { return key % mod; }

__kernel // kid 49
    void build_kernel(__global uint *rTableOnDevice, __global uint *rHashTable,
                      const uint rTupleNum, const uint sTupleNum,
  ...
  uint hashBucketSize =
      rTupleNum /
      rHashTableBucketNum; // number of tuples inserted into each bucket
  ...
  while (tid < rTupleNum) {
    ...
    if ((count = atomic_inc(&rHashTable[hash * (1 + hashBucketSize * 2)])) <
        hashBucketSize) // if this bucket is not full, add it
    {
...
__kernel // kid = 51
    void WAS_kernel(volatile __global WASEntry *was_buffer, const int wassize,
                    volatile __global uint *dummy_addr) {
  // WASEntry = 32B = 1 cache line (32B). Per-slot local copy + spin-loop.
  // dummy_addr[0] = prefetch hit counter, dummy_addr[1] = stop flag
  volatile uint temp = 0;
  uint count = 0;
  int idx = wassize - 1;

  volatile __global uint *last_tag[65536];
  for (int i = 0; i < wassize; i++)
    last_tag[i] = dummy_addr;

  while (dummy_addr[1] == 0) {
    __global uint *p1 = was_buffer[idx].p1;

    // Unchanged since last visit → advance to next slot
    while (p1 == last_tag[idx] || p1 == (__global uint *)dummy_addr) {
      if (dummy_addr[1] != 0) {
        dummy_addr[0] += count;
        return;
      }
