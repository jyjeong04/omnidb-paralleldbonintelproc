...
__kernel void // kid=20 — race prevention via was_per_wi (contiguous slot
              // ranges)
filterImpl_map_kernel(__global Record *d_Rin, int beginPos, int rLen,
  ...
  // M1 — WG 기준 partition only.  같은 WG는 같은 CU(=같은 호스트 스레드)에서
  // 순차 실행되므로 stride/lid 분할 없이도 race 없음 (PoCL CPU 가정).
  ...
  // 경량 flush — WG 의 슬롯 전체를 lid==0 한 명만 처리 (중복 flush 회피, 단일
  // LSB compare). (paper §4 의 "post dummy" 와 의미적 등가, post() 의 4 writes
  // 절약)
